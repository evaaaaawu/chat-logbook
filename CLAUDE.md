...
- Source directories (`~/.claude/`, `~/.codex/`, etc.) are read-only.
  Never write, modify, or delete anything under them.
...
Before changing anything that touches SQLite schemas, archive ingestion,
or FTS indexing — including any `*.db` files or storage layer code —
