...
- **Sort key is `first_seen_at` (createdAt), not derived `updatedAt`.** The benchmark sorts on `chats.first_seen_at`, a real indexable column, to isolate the cross-store cost. Sorting by "most recent activity" (`max(messages.ts)`) is the likely product default and is **not** covered here: it needs either a denormalized `updated_at` column on `chats` (kept current at ingest) or an aggregate that defeats the keyset index. Decide that separately before the list UI commits to an activity sort — the `ATTACH` shape above carries over, only the sort column and its index change. _(Resolved — see "Update: both time axes are denormalized" below.)_
...
## Update: both time axes are denormalized

The benchmark above sorts on `first_seen_at` (the ingest time) only to isolate the cross-store cost. The reader, though, displays createdAt as `min(messages.ts)` (the first message time) and updatedAt as `max(messages.ts)` (the most recent activity) — neither is `first_seen_at`. Sorting the page by `first_seen_at` while showing those derived values makes the cross-page order look wrong. Both axes are now reconciled with denormalized columns kept current at ingest, each backed by its own covering `(sortKey, id)` keyset index, so the paged order matches what the reader shows:

- **`updated_at` = `max(messages.ts)`** (issue #129, migration 0009). Initialized to `first_seen_at` for a chat with no messages; the write seam bumps it with `max(updated_at, ts)` on every message upsert.
- **`created_at` = `min(messages.ts)`** (issue #143, migration 0010). Initialized to `first_seen_at`; the write seam pulls it down with `min(created_at, ts)` on every upsert. Because `first_seen_at` is the ingest time and so `>=` every message ts, the running min equals `min(messages.ts)` once any message exists, and `first_seen_at` when none — matching the reader exactly. The createdAt keyset index is repointed from `first_seen_at` to `created_at`.

The `ATTACH` query shape, cursor design, and the rest of this ADR are unchanged — only the createdAt sort column and its index move. Issue #143 also adds an asc/desc `direction`: the keyset comparison (`<`/`>`) and `ORDER BY` flip together, and the covering `(sortKey, id)` indexes scan either way, so both directions stay a page-bounded index range scan.
